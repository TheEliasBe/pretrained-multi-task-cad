...
class RapidcadpyShaftDataset(Dataset):
    def __init__(
        self,
        config,
        tokenize=True,
        ...
        if config.data.use_cad_query_code:
            # Recursively load all Python files from the shaft dataset
            data_dir = pathlib.Path(config.data.root_dir) / "shaft" / "python"

            # Gather .py files (recursively)
            py_files = sorted([p for p in data_dir.rglob("*.py") if p.is_file()])
            ...
            for pf in py_files:
                ...
                # Use path relative to data_dir as a stable identifier
                rel_path = pf.relative_to(data_dir)
                deepcad_id = str(rel_path.stem)
            ...
            cadquery_df = (
                pd.DataFrame.from_records(records) if len(records) > 0 else None
            )
        ...
        if cadquery_df is not None:
            for idx, row in cadquery_df.iterrows():
                ...
                code = RapidcadpyShaftDataset.round_floats_in_code(code, digits=2)
    ...
    def create_splits(config, tokenize=True):
        ...
        dataset = RapidcadpyShaftDataset(config, tokenize=tokenize)
